...
export default function Home() {
  ...
  const handleRemoveImage = (imageId: string) => {
    const isUsed = timelineImages.some(img => img.imageAssetId === imageId);
    if (isUsed && !confirm('This image is used on the timeline. Removing it will also remove it from the timeline. Continue?')) {
      return;
    }

    setImageAssets(prev => prev.filter(img => img.id !== imageId));
    setTimelineImages(prev => prev.filter(img => img.imageAssetId !== imageId));
  };
